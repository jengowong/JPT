...
@NamedQueries({
    @NamedQuery(name="findAll",
        query="SELECT s FROM StockPriceEagerEagerImpl s ORDER BY s.id.symbol"),
    @NamedQuery(name="findJoin",
        query="SELECT s FROM StockPriceEagerEagerImpl s JOIN FETCH s.optionsPrices ORDER BY s.id.symbol"
    )
})

@Table(name="StockPrice")
public class StockPriceEagerEagerImpl implements Serializable, StockPrice {
    ...
    private Collection<StockOptionPriceEagerEagerImpl> optionsPrices;
    ...
    public StockPriceEagerEagerImpl() {
    }

    StockPriceEagerEagerImpl(StockPricePK pk) {
        id = pk;
    }
    ...
    public boolean equals(Object object) {
        if (!(object instanceof StockPriceEagerEagerImpl)) {
            return false;
        }
        StockPriceEagerEagerImpl other = (StockPriceEagerEagerImpl) object;
