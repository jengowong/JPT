...
@NamedQueries({
    @NamedQuery(name="findAll",
        query="SELECT s FROM StockPriceLazyLazyImpl s ORDER BY s.id.symbol"
        ),
    @NamedQuery(name="findJoin",
        query="SELECT s FROM StockPriceLazyLazyImpl s JOIN FETCH s.optionsPrices ORDER BY s.id.symbol"
    )
})

@Table(name="StockPrice")
public class StockPriceLazyLazyImpl implements Serializable, StockPrice {
    ...
    private Collection<StockOptionPriceLazyLazyImpl> optionsPrices;
    ...
    public StockPriceLazyLazyImpl() {
    }

    StockPriceLazyLazyImpl(StockPricePK pk) {
        id = pk;
    }
    ...
    public boolean equals(Object object) {
        if (!(object instanceof StockPriceLazyLazyImpl)) {
            return false;
        }
        StockPriceLazyLazyImpl other = (StockPriceLazyLazyImpl) object;
